...
Die aktuelle Situation stellt sich wie folgt dar: In E-Commerce-Unternehmen gibt es in der Regel verschiedene Kunden,
...
\shellinline{node_modules}-Verzeichnis.
...
der Plattform bereithält.\footpartcite{shopware-docker}
...
Da GitLab CI/CD mit verschiedenen\ \acrshort{vcs} genutzt werden kann, wird sich in der weiteren Konzeption auf die
...
\begin{wrapfigure}{r}{0.575\textwidth}
    ...
    \captioncite[Eigene Darstellung]{}{Visualisierung der konzipierten \acrshort{ci}-Pipeline}
...
ist: \shellinline{development},\ \shellinline{staging} und \shellinline{main}.
Die Development-Umgebung repräsentiert den Stand des \shellinline{development}-Branches und ist die erste Server-Instanz
...
Der \shellinline{staging}-Branch stellt die Basis für die Staging-Umgebung bereit.
Hierbei handelt es sich um eine stabile Umgebung, in die die erfolgreich getesteten Änderungen des
\shellinline{development}-Branches eingeführt werden, sodass der Auftraggeber des Shops diesen zustimmen kann, bevor
sie den Endkunden im Produktions-System erreichen.
Letztlich werden die vom Auftraggeber freigegebenen Änderungen gesammelt vom \shellinline{staging}-Branch in den
\shellinline{main}-Branch überführt.
...
Die Auswahl der genannten Tools basiert auf verschiedenen Faktoren.
...
zu können.\footpartcite[203--211]{duvall}
...
verteilte Container-Umgebungen genutzt werden kann.\footpartcite{gitlab-agent}
...
Software zu einem vorherigen, stabilen Stand zurückzuführen.\footpartcite[259--261]{humble}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/content/ci-architecture-concept}
    \captioncite[Eigene Darstellung nach]{docker}{Visualisierung der Architektur der konzipierten Strategie}
