...
Continuous-Software-Engineering fasst die Prinzipien der\ \acrfull{ci},
\ \acrfull{cde}, und\ \acrfull{cd} zusammen.
...
Die Disziplin umfasst Geschäftsstrategie und Planung, sowie die Entwicklung und den Betrieb der Software.
...
Nachfolgend werden die Bereiche der \hyperref[subsubsec:02-background-1-agil]{agilen Softwareentwicklung} und der\ \acrshort{ci},\ \acrshort{cde} und\ \acrshort{cd}
kurz erläutert.

\subsubsection{Agile Software Development} \label{subsubsec:02-background-1-agil}
...
\subsubsection{\acrlong{cd}}

\acrfull{cd} ist der nächste Schritt nach Continuous Delivery.
...
Der Zusammenhang zwischen\ \acrlong{ci},\ \acrlong{cde} und\ \acrlong{cd} wird in Abbildung\ \ref{fig:ci-cde-cd}
aufgezeigt.
...
\subsection{Begrifflichkeiten und Prinzipien von\ \acrlong{ci}} \label{subsec:02-background-2}
...
In 2010 betonen Humble und Farley die Wichtigkeit von\ \acrshort{ci} und behaupten, dass Software ohne\ \acrlong{ci}
als defekt gilt, bis sie als funktionierend nachgewiesen wird, während mit\ \acrshort{ci} Software mit
